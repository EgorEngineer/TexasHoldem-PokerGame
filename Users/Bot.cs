...
namespace Poker.Users
{
    public class Bot : UsersProperties
    {
        ...
        public Bot Fold(Bot user)
        {
            ...
            user.StatusLabel.Text = @"Вышел";
            user.ChipsTextBox.Text = @"Фишки : " + user.Chips;
            user.Turn = false;
            user.FoldTurn = true;
        ...
        public Bot Call(Bot user, ref int previous)
        {
            if (user.Chips > MainPoker.Call)
            {
                ...
                user.ChipsTextBox.Text = @"Фишки : " + user.Chips;
                user.StatusLabel.Text = @"Колл " + (MainPoker.Call + previous);
            ...
            MainPoker.Raising = false;
            user.Turn = false;
            user.StatusLabel.Text = @"Колл " + user.Chips;
        ...
        public Bot Raised(Bot user, ref int previous)
        {
            ...
            user.StatusLabel.Text = @"Повышает " + MainPoker.Raise;
            user.ChipsTextBox.Text = @"Фишки : " + user.Chips;
            ...
            user.Turn = false;
            previous = int.Parse(user.StatusLabel.Text.Substring(9));
            user = (Bot)MainPoker.GetStatus(user);
            return user;
        }
        ...
        public Bot Ph_1(Bot user, int minimum, int maximum, ref int previous)
        {
            ...
            {
                if (MainPoker.Call >= minimum && MainPoker.Call <= maximum ||  MainPoker.Call <= minimum && MainPoker.Call >= minimum / 2)
                {
                    user = Call(user, ref previous);
                }
